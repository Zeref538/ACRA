...
def detect_conflicts(
    ...
    for i in range(n_clusters):
        for j in range(i + 1, n_clusters):
            ...
            # Determine pair thresholds and red/green status
            ...
            if not use_fast:
                ...
                # Pass 1: Fast CIE76 check with a safe margin
                de_sim_76 = delta_e_cie76(ci_sim, cj_sim)
                if de_sim_76 >= pair_threshold + 15.0:
                    continue
                # Pass 2: Accurate CIEDE2000 check
                de_sim = delta_e_ciede2000(ci_sim, cj_sim)
            else:
                de_sim = delta_e_cie76(ci_sim, cj_sim)
            ...
            if not use_fast:
                de_orig_76 = delta_e_cie76(ci_orig, cj_orig)
                if de_orig_76 < 12.0:
                    continue
                de_orig = delta_e_ciede2000(ci_orig, cj_orig)
            else:
                de_orig = delta_e_cie76(ci_orig, cj_orig)
