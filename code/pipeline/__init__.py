...
def run_full_pipeline(
    ...
    # Pin background and person-class clusters — shifting them would alter
    # non-target regions like skin, walls, and sky, creating visible artifacts
    # even though the CNN did not flag those areas for correction.
    if use_segmentation and seg_class_names is not None:
        for i, name in enumerate(seg_class_names):
            nl = name.lower()
            if i == 0 or 'person' in nl or 'exclude' in nl:
                shifts[i] = 0.0
